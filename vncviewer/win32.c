...
static LRESULT CALLBACK keyboard_hook(int nCode, WPARAM wParam, LPARAM lParam)
{
  if (nCode >= 0) {
    ...
    if (is_win_hotkey(msgInfo->vkCode)) {
      release_lwin();
    }

    if (is_system_hotkey(msgInfo->vkCode, msgInfo->flags) || winkey_released == 0) {
